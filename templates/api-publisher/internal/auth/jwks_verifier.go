...
// JWKS local verification: the auth service signs access tokens RS256 with a
// kid header and serves its public keys at /.well-known/jwks.json; this API
// verifies tokens LOCALLY against a cached key set — no per-request hop to
// the auth service (which would make it a request-path SPOF). See the
// auth-service template for the issuing side, and CONTRACTS.md for the wire
// contract.
//
// Only RS256 is accepted here BY DESIGN. If your auth service can still mint
// HS256 during a migration, those tokens fail local verification — wire an
// HTTP-introspection fallback in the middleware if you need to accept them
// (the seam is documented in CONTRACTS.md).

const (
	jwksPath = "/.well-known/jwks.json"
	// minRefreshInterval rate-limits unknown-kid refreshes so a flood of
	// bad tokens cannot hammer the auth service (the whole point of local
	// verification is to stop depending on it per-request).
...
// Claims are the access-token claims this API consumes. Adjust the JSON tags
// and fields to match your auth service's token (userId/email/role/tokenType
// here). tokenType must be "ACCESS_TOKEN" (see Verify).
type Claims struct {
...
// NewJWKSVerifier derives the JWKS URL from the auth service base URL
